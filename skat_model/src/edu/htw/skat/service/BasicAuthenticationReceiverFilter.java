...
public class BasicAuthenticationReceiverFilter implements ContainerRequestFilter {
	...
	public void filter (final ContainerRequestContext requestContext) throws NullPointerException, ClientErrorException {
		...
		if (textCredentials != null) {
			final String credentials = new String(Base64.getDecoder().decode(textCredentials.substring(6)));
			
			final String[] credentialsPair = credentials.split(":"); 
			final String email = credentialsPair[0].trim();
			final String password = credentialsPair[1].trim();
			...
			if (requester != null) {
				final String rightPasswordHash = requester.getPasswordHash();
				final String leftPasswordHash = HashCodes.sha2HashText(256, password);
				
				if (rightPasswordHash.equals(leftPasswordHash)) {
